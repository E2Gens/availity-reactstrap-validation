...
describe('Date Validation', () => {
  ...
  describe('error message', () => {
    ...
    it('should use the custom format in the default message', () => {
      expect(fn('abc 123', context, {format: 'YYYY-MM-DD'}, input)).toEqual('Format needs to be YYYY-MM-DD');
    });

    it('should use the default format in the default message', () => {
      expect(fn('abc 123', context, undefined, input)).toEqual('Format needs to be MM/DD/YYYY');
    });
  ...
  describe('non-date input type', () => {
    ...
    it('should accept MM/DD/YYYY format by default', () => {
      expect(fn('10/12/2014', context, undefined, input)).toBe(true);
    });

    it('should accept ISO (YYYY-MM-DD) format by default', () => {
      expect(fn('2014-10-12', context, undefined, input)).toBe(true);
    });

    it('should allow format to be customized', () => {
      expect(fn('2014-12-14', context, {format: 'YYYY-MM-DD'}, input)).toBe(true);
    });

    it('should ensure the format matches the customized format', () => {
      expect(fn('10/12/2014', context, {format: 'YYYY-MM-DD'}, input)).toEqual('Format needs to be YYYY-MM-DD');
    });
  });
  ...
  describe('date input type', () => {
    ...
    describe('with browser date input type support', () => {
      ...
      it('should allow format to be customized', () => {
        expect(fn('10-12-2014', context, {format: 'DD-MM-YYYY'}, input)).toBe(true);
      });
    });
  });
});
